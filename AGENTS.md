...
- GUI 单文件模式：有密码 → 尝试提取，**失败静默回退伪转换**（绝不暴露"有载荷"信息；img 真转码，audio 改后缀）；无密码 → 伪转换。
...
- GUI 中文字体：`C:\Windows\Fonts\msyh.ttc`，失败回退 `simhei.ttf`；隐藏窗热键 `ImGui::IsKeyChordPressed(ImGuiMod_Ctrl|ImGuiMod_Shift, ImGuiKey_F)`；密码字段（img「镜头格式」/ audio「流派」）默认填充假数据（RAW / 流行）防"就它一个空着"露馅，`pwd_touched`/`genre_touched` 回调跟踪编辑状态——**未编辑过点确定一律视为空密码**（假数据绝不当真密码用），「恢复默认」重置 touched。
