...
- 构建：根目录 `build.bat`（ASCII-only）一次构建三个 exe，需 w64devkit 的 `g++` 在 PATH。GUI exe 用 `-mwindows`，CLI 不用。
...
- 测试依赖 Python 3.14 + Pillow + numpy + cryptography；`tests/tester.py` 的 `ROOT` 已相对化（仓库可移动）且 `summary()` 自动归档结果到 `tests/results/`；`tests/envelope.py` 为信封参考实现本地副本（test_cross 优先本地）；WAV 测试宿主 `test.wav`（44.1kHz/16bit/stereo 60s，`tests/gen_wav.py` 确定性合成，缺失时 W0 自动生成）；`tests/gen_pdf.py`（md → HTML → Word COM → PDF）重建根目录 `使用说明书.pdf` / `技术报告.pdf`（md 源在 `docs/`）。
...
- **禁止修改 `stb/` 和 `imgui/`，禁止引入新依赖**（只许系统库 + 现有第三方）。
- 项目不是 git 仓库；根目录 .exe / .zip / `Creeper便携版/`（分发目录，含 PDF 说明书）都是交付物，别删别乱动，exe 变更后需同步便携版。
