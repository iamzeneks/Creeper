...
Produces three executables into `res/` (release package, alongside the guide PDFs): `creeper_cli.exe` (console) and `creeper_img.exe` / `creeper_audio.exe` (GUI, `-mwindows`). Third-party deps `imgui/` and `stb/` ship in `third_party/`.
...
├─ res/               release package (gitignored: exes + guide PDFs)
