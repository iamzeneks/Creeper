...
Produces three executables in the repository root: `creeper_cli.exe` (console) and `creeper_img.exe` / `creeper_audio.exe` (GUI, `-mwindows`). Third-party deps `imgui/` and `stb/` ship in `third_party/`.
...
│  └─ build.bat       build script (w64devkit g++; outputs exes to repo root)
├─ third_party/       imgui / stb (open-source deps, do not modify)
├─ docs/              specs, test report, product docs (markdown + generated PDFs)
