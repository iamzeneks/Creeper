...
特工电影里，情报从不装进带密码锁的公文包——它们藏在明处：一段消息混进报纸的分类广告，谁都能买到那份报，谁都不会多看一眼。Creeper 干的就是这行：把你的合同、证件、密码串**加密后藏进一张照片、一首歌或一段录音**。照片照样晒、音乐照样播、云端照样同步，旁观者只当是寻常文件；而藏在里面的东西，只有拿着密码的人取得出。
