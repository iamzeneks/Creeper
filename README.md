...
特工电影里，情报不装在密码锁公文包里，而是混进报纸的分类广告——谁都能买到那份报，谁都不会多看一眼。Creeper 也是这么干的：把你的合同、证件、密码串**加密后藏进一张照片、一首歌或一段录音**。从外表看，它们就是再普通不过的文件，打开、播放、同步网盘都一切正常；藏在里面的东西，只有知道密码的人才能取出来。

**是**：
- 把任意文件加密后，无损嵌入 PNG / MP3 / WAV 载体
- 只有密码正确才能取出原文件；密码错误时与"里面没藏东西"不可区分

**不是**：
- **不是加密压缩包**。加密 zip、加密 PDF、加密磁盘，等于在门口挂块牌子："此处存有重要文件"——看见的人立刻知道"有东西"。Creeper 藏完之后，载体只是普通媒体——防的不只是"解不开"，而是**根本不知道有东西可解**。

几分钟就能上手，见下方的[快速上手](#快速上手)。
...
```bat
:: 加密后藏进载体：机密.pdf → 输出.png（载体支持 PNG/MP3/WAV，输出同格式）
creeper_cli embed 载体.png 机密.pdf 输出.png 你的密码

:: 取回：输出.png → 输出目录/机密.pdf
creeper_cli extract 输出.png 输出目录 你的密码
```

也可以直接用伪装 GUI：打开 `creeper_img.exe`（图片）或 `creeper_audio.exe`（音频），把文件拖进列表、点「开始转换」即可（完整说明见「[GUI](#gui伪装模式)」一节）。更多命令——加密信封、多宿主分片、载荷检测等——见下方「[CLI 完整用法](#cli-完整用法)」。
...
`creeper_img.exe`（图片"格式转换"）与 `creeper_audio.exe`（音频"格式转换"）界面呈现为普通格式转换器；真实功能经隐藏入口使用（`Ctrl+Shift+F` 呼出隐藏窗）。

- **单文件模式**：有密码 → 尝试提取，失败静默回退伪转换；无密码 → 伪转换。
- **2 个文件（1 宿主 + 1 载荷）**：勾不勾选「硬件加速」都加密嵌入。
- **3+ 个文件**（由「硬件加速」勾选分派）：
  - 勾选 = 加密嵌入（列表最后一个文件 = 载荷，其余 = 宿主，自动分片到多个宿主；可用「上移」「下移」把载荷挪到最后）
  - 不勾选 = 解密提取（全部 = 宿主，乱序自动拼接还原）
- **无密码**（多文件）：一律假装批量转换，不暴露任何信息。
- **隐藏窗**：另提供「编码质量」（填充率上限）与「位深」（WAV 深度 1/2/3）调节。
