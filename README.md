...
`creeper_img.exe`（图片"格式转换"）与 `creeper_audio.exe`（音频"格式转换"）界面呈现为普通格式转换器；真实功能经隐藏入口使用（`Ctrl+Shift+F` 呼出隐藏窗）。单文件模式：有密码 → 尝试提取，失败静默回退伪转换；无密码 → 伪转换。多文件模式（2+）由主界面「硬件加速」勾选分派：**2 个文件 = 1 宿主 + 1 载荷，勾不勾选都加密嵌入**；**3+ 文件时勾选 = 加密嵌入**（列表最后一个文件 = 载荷，其余 = 宿主，自动分片到多个宿主；可用「上移」「下移」把载荷挪到最后），**不勾选 = 解密提取**（全部 = 宿主，乱序自动拼接还原）；无密码时多文件一律假装批量转换（不暴露任何信息）。隐藏窗另提供「编码质量」（填充率上限）与「位深」（WAV 深度 1/2/3）调节。
