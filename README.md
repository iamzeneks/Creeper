...
你有一份不想让人看见的文件：身份证扫描件、一份合同、一串密码……直接放网盘，文件名就露馅；打包成加密压缩包，一个 `.zip`/`.rar` 又会引来注意。Creeper 把这个文件**加密**后藏进一张你再普通不过的图片、一段音乐或一个 WAV 音频里——载体看起来毫无异常，照样能看、能分享、能同步到云端，而真正的内容只属于知道密码的你。
...
**不是**：
- 不是压缩工具——载体是正常媒体，不会变小，也不是压缩包
- 不是网盘/云存储——不替你保存文件，只帮你"藏"
- 不是加密聊天软件——不负责传输
