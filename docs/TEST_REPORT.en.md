...
- **System under test**: `creeper_cli.exe` / `creeper_img.exe` / `creeper_audio.exe` at the repository root (built 2026-08-19 25:10; implemented features are listed by stage under "Conclusion" below)
...
- **GUI-4 (2026-08-20)**: 2 files (1 carrier + 1 payload) always embed regardless of whether the checkbox is ticked; password fields default to fake data (RAW / Popular), unedited fields treated as empty password; capacity pre-check before embedding (warn in advance when exceeding the tier).
