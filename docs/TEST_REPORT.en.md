...
- **Test time**:
  ...
  - Full regression after error-message disguise + popup terminology cleanup: 2026-08-20 18:50
  - Full regression after CLI cross-platform port (Linux/macOS source packages): 2026-08-20 19:30
...
- **GUI-6 (2026-08-20)**: single-file extraction routed through split-reconstruct (first `split_extract({host})`, falling back to plain extraction); wrong password / no payload → popup「转换失败：编码错误」(no stego terminology exposed), genuinely damaged / unsupported format →「转换失败：文件已损坏或格式不受支持」; popup terminology cleanup (payload → source file); full regression 170/170.
- **PORT (2026-08-20)**: CLI cross-platform port (Linux/macOS source packages + Windows CLI binary release) — file IO / directory creation gained POSIX branches (`fopen`/`mkdir`/`stat`); randomness unified on `std::random_device` (Win→RtlGenRandom / Linux→getrandom / macOS→arc4random, no bcrypt import), png/wav per-pixel high-frequency randomness switched to a one-time-seeded `mt19937_64` fast PRNG; `cli_main.cpp` uses argv directly on POSIX (UTF-8); added repository-root `build.sh` (auto-picks g++/clang++) and build tutorial `docs/CLI_BUILD.md`; Windows full regression **170/170** passes (zero Win-side regression); the Linux/macOS branches were not compiled on a real machine (no Cygwin/WSL locally — POSIX branches were statically reviewed; real-machine verification pending).
...
- **Implementation**:
  ...
  - **GUI**: removed `has_payload_fn` pre-check (no more "one-shot detection" path); single file + password → directly attempt extract (**`split_extract({host})` first, falling back to plain extraction; GUI-6 fix, see below**), on total failure silently fall back to fake conversion (never expose "has payload" info); embed too-large → "转换失败：文件过大，无法完成转换" dialog
...
- **Verification**: `test_split.py` **22/22** (S1 2PNG+WAV three-block round-trip, S2 out-of-order reassembly, S3 PNG+WAV+MP3 mixed, S4 missing block → missing error, S5 wrong password fails, S6 single-block extract fails + has=0, S7 capacity over-sum error, S8 depth2 sharding, S9 empty password, S10 ordinary single-carrier file unsplit treated as no payload, **S11 single-carrier split round-trip restores identical bytes + single-carrier split file must fail plain extract (GUI-6 regression)**); full regression **170/170**
- **GUI**: multi-file (2+) with password dispatched by「硬件加速」(hardware acceleration) checkbox — checked=embed (last=payload), unchecked=extract (all carriers);「上移」「下移」buttons for ordering; no password always pretends batch conversion (no disclosure); single-file extraction routed through split-reconstruct (GUI-6 fix: `split_extract({host})` first, falling back to plain extraction)
...
- **Fix**: `audio_extract` / `img_extract` now try **`split_extract({host})` first** (`split_extract` handles count=1 single block: concatenate by index → whole envelope → GCM-authenticated restore), falling back to plain extraction (for CLI `embed` plain envelopes) and only then to error → fake-conversion fallback. Disguise copy: wrong password / GCM auth failure / no payload uniformly show 「转换失败：编码错误」(wrong-password and no-payload remain indistinguishable, but no steganography concept is exposed); genuine corruption / unsupported format show 「转换失败：文件已损坏或格式不受支持」.
