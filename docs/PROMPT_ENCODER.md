...
- **方案**：不碰 RIFF/fmt 区，只改 data 区每个样本的最低 1 bit：
  - 支持 PCM（format=1）8-bit 无符号 / 16-bit 有符号小端，任意声道数；样本顺序 = data 区顺序（多通道交错）
  - 嵌入 = **±1（LSB matching，非强制 LSB）**：目标位与样本 LSB 相同 → 不动；不同 → 样本 ±1（方向随机 50/50；8-bit 边界 0/255、16-bit 边界 ±32768 单方向）
  - 其他格式（float/A-law/压缩）→ 报错 `unsupported wav format (need PCM)` / `unsupported wav bit depth (need 8 or 16)`
- **隐写头**（无魔数、无 seed 字段，与 PNG 完全同构）：
  ```
  name_len 2B  大端（原始文件名长度）
  name     原始文件名 UTF-8（name_len 字节）
  env_len  4B  大端（信封长度）
  ```
- **散布种子由密码派生**：`crypto_steg_seed(password, "creeper-wav")`；头+信封体整体按 seed 用 xorshift64 伪随机散布（纯内容无关序列，占用线性探测；提取端从嵌入后文件精确重放）
- **容量检查**（两道，与 PNG 同）：填充率上限（`fill_limit_pct` 默认 15，CLI `--cap` 可覆盖，`0`=不限）→ `payload too large: exceeds N% of host capacity (use --cap to raise the limit)`；绝对容量（样本数 < 位数）→ `payload too large for host wav capacity`
...
- **容量参考**：44.1kHz/16bit/stereo 每样本 1 bit → 330,750 B/分钟（100%）；默认 15% ≈ **99 KB/分钟**（`--cap` 可放开）；对比 MP3 ≈ 7.2 KB/整首
- 接口（与 png 同名同构，含 fill_limit_pct）：
  ```cpp
...
- embed/extract/has 的宿主类型按扩展名分发：.png → png_steg、.wav → wav_steg、.mp3 → mp3_steg；其他扩展名 → `unsupported host file type (need .png, .wav or .mp3)`
...
- 文件列表（可多选/拖拽添加，显示路径列表）+「添加文件」「移除选中」「清空列表」
...
- 列表里 **2 个文件**（第一个=宿主，第二个=载荷）→ **加密模式**：embed(host, payload, 密码, 输出目录/宿主名_已转换.png|mp3)。载荷文件不输出
...
- **img 字段**（每次打开重置为以下预制值）：厂商=`Canon`、型号=`Canon EOS R6 Mark II`、镜头=`RF24-70mm F2.8 L IS USM`、**镜头格式 = ←密码填这里**、光圈=`f/2.8`、快门=`1/250s`、ISO=`400`、白平衡=`自动`、拍摄日期=`2025-06-15 14:32`、分辨率=`5472×3648`、软件=`Adobe Lightroom Classic 13.2`、备注=`Demo photo, all rights reserved.`
- **audio 字段**：标题=`Midnight Drive`、艺术家=`Neon Harbor`、专辑=`City Lights EP`、年份=`2023`、**流派 = ←密码填这里**、音轨=`3`、比特率=`320kbps`、采样率=`44100Hz`、注释=`Licensed under CC BY-NC 4.0`
- 按钮：`确定`（把密码存入内存全局变量）、`取消`、`恢复默认`（重新填充预制值）
- 密码字段用 `ImGui::InputText(..., ImGuiInputTextFlags_Password)`（显示为 ••••）
...
g++ -O2 -std=c++17 -DUNICODE -D_UNICODE -mwindows -I. -Iimgui -Iimgui\backends -Istb ^
  img_app.cpp common_ui.cpp crypto.cpp png_steg.cpp mp3_steg.cpp ^
...
rem creeper_cli 不加 -mwindows，链接同上 → creeper_cli.exe
```
...
5. GUI 两个 exe 能启动、显示中文界面（人工确认）
6. 编译零警告（尽力而为）
7. 抽查三个 exe 二进制：不得含 `CREEPER1`/`CRPR`/`CRP`/`creeper` 明文（GUI exe 全查；CLI 仅允许 usage 帮助文本出现 `creeper_cli`）
