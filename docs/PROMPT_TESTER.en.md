...
- **Password-field disguise check (GUI-5)**: the hidden window's「镜头格式」/「流派」fields default to **plaintext random fake text** (randomly picked from a real format/genre wordlist, not asterisks); after typing, the display layer is masked by random alphanumerics (OCR can assert the default text is not `***`/`••••`); a small grey ✕ exists at the right inside the input box and clears it on click; clicking「开始转换」during/in response to conversion must not trigger an ImGui assertion (`imgui.cpp` `EndDisabled`) crash
