...
- CLI 用法（见 PROMPT_ENCODER.md §5）：
  `creeper_cli seal <in> <out> <password>` / `open <env> <out> <password>` / `embed <host> <payload> <out> <password> [--cap N]` / `extract <host> <outdir> <password>` / `has <host> <password>`（输出 1/0，密码错或无载荷均为 0）
...
- **填充率上限（默认 15%）**：超限载荷 → 报 `payload too large: exceeds 15%...` 且无输出；同载荷 `--cap 100` → 成功往返；绝对容量超限（> 样本数）→ 报错
...
- `tests/test_crypto.py`、`tests/test_png.py`、`tests/test_mp3.py`、`tests/test_cross.py`（Python，可直接重跑）
- `tests/test_gui.ps1`
