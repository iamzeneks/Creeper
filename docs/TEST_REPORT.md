...
**结论**：核心功能（加密往返、跨语言互解、PNG/MP3/WAV 隐写往返、检测、GUI）全部可用；**NO-MAGIC-1 加固（2026-08-18）完成**：PNG/MP3 隐写头移除固定魔数（载荷存在性改由 GCM 认证判定，`has` 需密码参数）、exe 内 creeper 相关字符串 XOR 混淆（三个 exe 二进制抽查：GUI exe 无任何 `creeper` 明文，CLI 仅剩 usage 帮助文本）、PNG 新增默认 15% 填充率上限（CLI `--cap` 覆盖，防统计检测）。原 OBS-1（`has` 对空文件返回 0）随无魔数改造正式化为合理语义（空文件 = 无载荷 = 0）。**后续更新**：2026-08-18 GUI 布局修复（窗口固定尺寸 + DPI 适配），见 §9。**第二梯队加固（NO-MAGIC-2，2026-08-18 晚）完成**：PNG 隐写头移除明文 seed 字段（散布种子改由密码派生 `crypto_steg_seed(password,"creeper-seed")`，头+体整体散布，消除明文固定位置）；MP3 帧头辅助位整体 XOR 密码派生 keystream（`"creeper-ks"`，仅在帧层消费一次）；PNG 新增自对抗重嵌（直方图 L1 超阈值按 undo 恢复重嵌 ≤3 次 + 写盘前逐位自检）；自删 bat 更名 `msimg32_upd.bat`。期间发现并修复两个自建问题（见 §10：PNG 散布不一致、MP3 双重 keystream），全量回归 88/88。**WAV-1 新载体（2026-08-19 凌晨）完成**：新增 PCM（8/16-bit）无损 LSB 隐写（每样本 1 bit，±1 修改，RIFF/fmt 区零改动，位流与散布机制与 PNG 同构），CLI/GUI 集成（.wav 宿主自动分发），容量 ≈ 661KB/分钟（100%）/ 99KB/分钟（15% 上限）——**大文件首选 WAV 载体**；全量回归 114/114。**GUI-1 关于弹窗改造（2026-08-19 晚）完成**：关于页由 ImGui 共享窗口假弹窗改为原生 Win32 模态对话框（`XhAboutDlg`，400×254，`EnableWindow` 禁主窗 + 自跑模态循环），按钮/确认框/自毁链路全部真实验证（含副本自毁删 exe），见 §12；全量回归 114/114。**OBS-2 压缩率修复（2026-08-19 深夜）完成**：内置 DEFLATE 从"固定哈夫曼"升级为 **LZ77 + 三路块编码取最短**（固定哈夫曼/动态哈夫曼/存储块），与 Python zlib 差距从 ~5.4% 收敛到 ~0–17%（动态块，极端重复文本用例 1.9× 为 LZ77 匹配策略差异）；PNG 15% 上限实测可嵌入 **229,997 B**（修复前 218,476 B）；`--cap 100` 绝对上限实测 **≈1,535,800 B**（修复前 1,456,726 B）；开发中定位并修复 3 个内部 bug（哈夫曼长度队列计数、canonical 码递推、重复长度锚点），见 §13；**测试基建重建**：tester.py ROOT 相对化 + 结果自动归档 `../tests/results/`、`../tests/envelope.py` 本地化（test_cross 优先本地）、`../tests/gen_wav.py`（宿主自动生成）、`test_wav.py` 新增 8-bit/float 用例（26→31）、`../tests/gen_pdf.py`（md → PDF，Word COM）、`../tests/run_all.bat`（一键全量）、GUI 专项脚本固化（`test_gui_about.ps1` 关于弹窗真实点击 / `test_gui_quality.ps1` 隐藏窗 OCR）；**GUI 关于按钮真实鼠标点击验证通过**（DPI-aware 进程注入：点击 → `XhAboutDlg` 弹出 600×381 物理（400×254×1.5）→ 点「关闭」→ 弹窗销毁 + 焦点恢复主窗）；**GUI-2 编码质量（2026-08-19 深夜）完成**：隐藏窗新增「编码质量」下拉（标准 15% / 高 30% / 超高 50% / 极限 100%），映射嵌入填充率上限（GUI 双文件嵌入由此可突破默认 15%，等效 CLI `--cap`，伪装为格式转换器的"编码质量"选项，主界面无任何新增暴露）；**WAV-2 高容量模式（2026-08-19 深夜）完成**：WAV 承载深度可配置（`--depth 2` / GUI 音频隐藏窗「位深」= 高 24bit），每样本 2 bit 容量 ×2（134MB WAV：15% ≈ 1.3MB、100% ≈ 8.8MB），样本差 ≤3（-78dBFS 不可闻）；隐写头新增 1B depth 字段（无魔数原则保持），解析新格式优先、GCM 失败回退 v1.0 旧格式（旧文件全兼容，新文件旧代码视为无载荷）；全量回归 **137/137**。**WAV-3 三档深度（2026-08-19 深夜）完成**：WAV 承载深度扩展至 3（`--depth 3` / GUI「位深」超清 32bit），每样本 3 bit 容量 ×3（134MB WAV：15% ≈ 1.98 MB、100% ≈ 13.2 MB），样本差 ≤7（-69dBFS 仍不可闻）；修复 read_byte 逐字节消费在 depth=3 时的跨样本位溢出（3 样本=9 位>8，第 9 位丢失导致解析失败）——改为逐位缓冲消费，depth 1/2 位序不变（旧文件兼容）；**GUI 测试前台加固**：test_gui_quality/test_gui_about 增加窗口置顶 + 点击激活（绕过 Windows 前台锁定），消除被资源管理器遮挡导致的偶发失败；全量回归 **148/148**。**SPLIT 大文件分片多宿主（2026-08-20 凌晨）完成**：新增 `split`/`unsplit`（CLI）与「硬件加速」勾选分派（GUI）——先整体 seal 再切分密文，单块过不了 GCM 认证（「无载荷」语义），收齐所有块按编号拼接后整体认证还原（单宿主截获无意义）；块头 = 密码派生 magic（`crypto_steg_seed(password, "creeper-split")`）+ index/count/chunk_len，无明文特征；容量分配 = 每宿主 `floor(容量×填充率) − 头开销` 顺序填装（MP3 按 100%）；三宿主模块新增 embed_stream/read_stream/capacity 接口（原 embed/parse 逻辑提取复用，单宿主行为零变化）；GUI 多文件模式改为「硬件加速」勾选分派（勾选=嵌入、最后一个=载荷；不勾选=提取、全部=宿主）+「上移」「下移」排序按钮，无密码一律假装批量转换；全量回归 **166/166**。
